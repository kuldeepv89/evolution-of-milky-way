      ...
      SUBROUTINE MW(at1,atauin,aSDR,aTTspi,
     $aAFEHv,aaspiv,aAMFv)
      PARAMETER (NMAX=37) 
...
CCC         
      real at1,atauin,avnsol1,avnsol2,aSDr
      real t1,t2,tauin
...
Cf2py intent(in) at1, atauin, aSDR
...
c        ! BR=(VMR-GAS)/(T2*(1.-EXP(-(ETA-TAUIN)/T2)))
         BR=(VMR-GAS)/(ETA-TAUIN)
         BRO=BR
      END IF
...
c      !APUNT(I)=AR/EXP(T(I)/T1)+BR/EXP(TTAU/T2)
         APUNT(I)=AR/EXP(T(I)/T1)+BR
...
c!
c       SRT(I)=AR*T1*(1.-1./EXP(T(I)/T1))+BR*T2*(1.-
c     $      1./EXP(TTAU/T2))+GAS



      SRT(I)=AR*T1*(1.-1./EXP(T(I)/T1))+BR*TTAU+GAS
      
c      SMS(I)=ARO*T1*(1.-1./EXP(T(I)/T1))+BRO*T2*(1.-
c     $     1./EXP(TTAU/T2))+GAS

      SMS(I)=ARO*T1*(1.-1./EXP(T(I)/T1))+BRO*TTAU+GAS
c      GRAT(I)=VNSOL
...
C      IF (T(I).GE.2.0.AND.T(I).LE.2.05) GO TO 9364
      ...
      TM=T(I)+DTEM/2.
c      SRP=AR*T1*(1.-EXP(-TM/T1))+BR*T2*(1.-EXP(-(TM-TAUIN)/T2))+GAS
        SRP=AR*T1*(1.-EXP(-TM/T1))+BR*(TM-TAUIN)+GAS  
c      SMSP=ARO*T1*(1.-EXP(-TM/T1))+BRO*T2*(1.-EXP(-(TM-TAUIN)/T2))+GAS
      SMSP=ARO*T1*(1.-EXP(-TM/T1))+BRO*(TM-TAUIN)+GAS
      AP=AR*EXP(-TM/T1)+BR
      FAP=AP/SG
      DO    L=1,NMAX
         XP(L)=GP(L)/GASP
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
 ...
 658  continue
      gasp=0.
      do   j=1,nmax
c         gp(j)=g(i,j)+wi1(j)*dtem+ar*t1*exp(-t(i)/T1)*
c     $        (1.-exp(-dtem/T1))*
c     $        xinf(j)/sg+br*t2*exp(-t(i)/t2)*
c     $        (1.-exp(-dtem/t2))*xinf(j)/sg


             gp(j)=g(i,j)+wi1(j)*dtem+ar*t1*exp(-t(i)/T1)*
     $        (1.-exp(-dtem/T1))*
     $        xinf(j)/sg+br*dtem*xinf(j)/sg
...
C XU MASSA ESTREMO SUPERIORE
 ...
 40   continue
      ...
      tt=t(i)
      to=t(i-1)
c      sr1=ar*t1*(1.-exp(-to/t1))+br*t2*(1.-exp(-(to-tauin)/t2))+gas
c      sr2=ar*t1*(1.-exp(-tt/t1))+br*t2*(1.-exp(-(tt-tauin)/t2))+gas



      sr1=ar*t1*(1.-exp(-to/t1))+br*(to-tauin)+gas
      sr2=ar*t1*(1.-exp(-tt/t1))+br*(tt-tauin)+gas
...
c      !br=(vmr-gas)/(T2*(1.-exp(-(ETA-TAUIN)/T2)))
     ...
     $     EXP(-(TM(L2)-TAUIN)/T2))+GAS
       ...
       SRTM=AR*T1*(1.-EXP(-TM(L2)/T1))+BR*(TM(L2)-TAUIN)+GAS
      SMSM=ARO*T1*(1.-EXP(-TM(L2)/T1))+BRO*(TM(L2)-TAUIN)+GAS
...
c      SMSM=ARO*T1*(1.-EXP(-TM(N3)/T1))+BRO*T2*(1.-EXP(-TM(N3)/T2))+GAS
     ...
     $     EXP(-(TM(N3)-TAUIN)/T2))+GAS
       ...
       SRTM=AR*T1*(1.-EXP(-TM(N3)/T1))+BR*(TM(N3)-TAUIN)+GAS
      SMSM=ARO*T1*(1.-EXP(-TM(N3)/T1))+BRO*(TM(N3)-TAUIN)+GAS
...
C Il primo pezzo di integrale e` risolto...
 ...
 6    XSTIN(L)=X(I,L)+(XP(L)-X(I,L))/DTEM*(TSTAR-TDIZ)
     ...
     $     +GAS
       ...
       SSTAR=AR*T1*(1.-EXP(-TSTAR/T1))+BR*(TSTAR-TAUIN)
     $+ GAS
      SMTAR=ARO*T1*(1.-EXP(-TSTAR/T1))+BRO*(TSTAR-TAUIN)
     $+GAS
...
c                           !br=(vmr-gas)/(T2*(1.-exp(-(ETA-TAUIN)/T2)))
     ...
     $                       (TMT-TAUIN)/T2))  +GAS
                         ...
                         SRTM=AR*T1*(1.-EXP(-TMT/T1))+BR*
     $                       (TMT-TAUIN)+GAS

                        SMSM=ARO*T1*(1.-EXP(-TMT/T1))+BRO*
     $                       (TMT-TAUIN)  +GAS
                        GRATM=(GRAT(INF)+GRAT(ISUP))*0.5
...
c                     !br=(vmr-gas)/(T2*(1.-exp(-(ETA-TAUIN)/T2)))
     ...
     $                 GAS
                   ...
                   SSTAR=AR*T1*(1.-EXP(-TSTAR/T1))+BR*
     $                 (TSTAR-TAUIN)+
     $                 GAS
                  SMTAR=ARO*T1*(1.-EXP(-TSTAR/T1))+BRO*
     $                 (TSTAR-TAUIN)+
     $                 GAS
